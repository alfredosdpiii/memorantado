...
-- bm25 helpers
--
-- tsvector_token_count: number of token instances (positions) in a tsvector.
-- Used for the per-row search_length columns and the fts_stats corpus counts.
-- tsvector_prefix_freq: instances of lexemes with a given prefix (the FTS5
-- phrase-frequency term for a prefix query). Both parse the tsvector text
-- representation; lexemes never contain spaces/commas.
--------------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION tsvector_token_count(v TSVECTOR)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $func$
  SELECT COALESCE(SUM(array_length(string_to_array(entry, ','), 1)), 0)::INTEGER
  FROM unnest(string_to_array(v::text, ' ')) AS entry
$func$;

CREATE OR REPLACE FUNCTION tsvector_prefix_freq(v TSVECTOR, prefix TEXT)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $func$
  SELECT COALESCE(SUM(array_length(string_to_array(entry, ','), 1)), 0)::INTEGER
  FROM unnest(string_to_array(v::text, ' ')) AS entry
  WHERE starts_with(btrim(split_part(entry, ':', 1), ''''), prefix)
$func$;
...
-- Per-row token-instance counts (D in the bm25 formula). Added via ALTER so
-- databases created by older schema versions pick them up too.
ALTER TABLE episodes ADD COLUMN IF NOT EXISTS search_length INTEGER
  GENERATED ALWAYS AS
  (tsvector_token_count(to_tsvector('english',
     COALESCE(actor, '') || ' ' || role || ' ' || content || ' ' || COALESCE(source, ''))))
  STORED;
ALTER TABLE semantic_memories ADD COLUMN IF NOT EXISTS search_length INTEGER
  GENERATED ALWAYS AS
  (tsvector_token_count(to_tsvector('english',
     kind || ' ' || subject || ' ' || predicate || ' ' ||
     COALESCE(object, '') || ' ' || content)))
  STORED;
ALTER TABLE claim_versions ADD COLUMN IF NOT EXISTS search_length INTEGER
  GENERATED ALWAYS AS
  (tsvector_token_count(to_tsvector('english',
     kind || ' ' || subject || ' ' || predicate || ' ' ||
     COALESCE(object, '') || ' ' || content)))
...
--
-- The pg bm25 channel replicates FTS5's bm25() scoring (see channels.ts),
-- which needs whole-table row and token counts (FTS5 reads them from its
-- docsize index). Triggers keep this table in sync; token counts are the
-- number of tsvector positions, which excludes stopwords (documented
-- deviation from FTS5's porter token counts).
--------------------------------------------------------------------------------
...
-- Backfills stats for databases that already had data before this table
-- existed; a no-op on fresh or already-initialized databases.
INSERT INTO fts_stats (table_name, row_count, token_count)
SELECT 'episodes', count(*), COALESCE(SUM(search_length), 0)
FROM episodes
ON CONFLICT (table_name) DO NOTHING;

INSERT INTO fts_stats (table_name, row_count, token_count)
SELECT 'semantic_memories', count(*), COALESCE(SUM(search_length), 0)
FROM semantic_memories
ON CONFLICT (table_name) DO NOTHING;

INSERT INTO fts_stats (table_name, row_count, token_count)
SELECT 'claim_versions', count(*), COALESCE(SUM(search_length), 0)
FROM claim_versions
ON CONFLICT (table_name) DO NOTHING;

CREATE OR REPLACE FUNCTION memorantado_fts_stats_update()
RETURNS trigger
LANGUAGE plpgsql
AS $func$
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE fts_stats SET
      row_count = row_count + 1,
      token_count = token_count + NEW.search_length
    WHERE table_name = TG_TABLE_NAME;
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE fts_stats SET
      row_count = row_count - 1,
      token_count = token_count - OLD.search_length
    WHERE table_name = TG_TABLE_NAME;
    RETURN OLD;
  ELSE
    UPDATE fts_stats SET
      token_count = token_count - OLD.search_length + NEW.search_length
    WHERE table_name = TG_TABLE_NAME;
    RETURN NEW;
  END IF;
END;
...
DROP TRIGGER IF EXISTS episodes_fts_stats ON episodes;
CREATE TRIGGER episodes_fts_stats
AFTER INSERT OR UPDATE OR DELETE ON episodes
FOR EACH ROW EXECUTE FUNCTION memorantado_fts_stats_update();

DROP TRIGGER IF EXISTS semantic_memories_fts_stats ON semantic_memories;
CREATE TRIGGER semantic_memories_fts_stats
AFTER INSERT OR UPDATE OR DELETE ON semantic_memories
FOR EACH ROW EXECUTE FUNCTION memorantado_fts_stats_update();

DROP TRIGGER IF EXISTS claim_versions_fts_stats ON claim_versions;
CREATE TRIGGER claim_versions_fts_stats
